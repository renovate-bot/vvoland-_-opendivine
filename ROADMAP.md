...
- [ ] Engine-faithful collision — [formats/collide](re_docs/formats/collide.md)
  - [ ] cube from `anchor_x/y` + `width` + `x_extent` + `z_height` (drop the square-AABB approximation)
  - [ ] mover-vs-cube distance test; `type` gating
  - [ ] block/slide resolution matching the `0x414/0x415` cluster behaviour
